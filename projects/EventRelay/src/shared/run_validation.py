"""Persist run artifacts and validate them against Software-On-Demand schemas."""
...
VALIDATOR_ROOT = (PROJECT_ROOT.parent / "software-on-demand").resolve()
                ...
                outputs=[
                    {
                        "name": "action_plan",
                        "type": "json",
                        "value": {"actions_extracted": len(actions)},
                    }
                ],
            )
        )
...
def _build_step_graph(video_result: Dict[str, Any], run_id: str, base_time: datetime) -> Dict[str, Any]:
    ...
    steps = _make_steps(video_result, base_time)
    ...
    return {
        ...
        "run_context": {
            ...
            "stack": {"language": "python", "framework": "eventrelay"},
        },
        "steps": steps,
        "edges": edges,
    }
        ...
        events.append(
            {
                ...
                "payload": {
                    ...
                    "artifact_type": "file",
                    "uri": f"memory://transcripts/{video_result.get('metadata', {}).get('video_id', 'unknown')}.json",
                    "description": "Transcript segments captured from the source video.",
                },
            }
        )
        ...
        events.append(
            {
                ...
                "payload": {"name": "actions.extracted", "value": len(actions), "unit": "count"},
            }
        )
        counter += 1
...
def persist_run_artifacts(video_result: Dict[str, Any], *, source: str = "unknown", logger: Optional[Any] = None) -> Path:
    ...
    try:
        ...
        if callable(log_fn):
            ...
            print(f"[run-validation] Validation failed for {run_dir} (exit code {exc.returncode})")
        raise
